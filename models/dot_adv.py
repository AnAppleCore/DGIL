...
class Learner(BaseLearner):
    ...
    def incremental_train(self, data_manager: Union[DataManager, DomainDataManager] = None):
        ...
        if self.dot_epochs > 0:
            self._train_transformation(self.train_loader) # train u->i and i->u transformations
        if self.ca_epochs > 0:
    ...
    def _train_transformation(self, train_loader):
        if len(self._multiple_gpus) > 1:
            self._network = self._network.module
        self._network.reset_discriminator()

        gen_params = list(self._network.ui_domain_transform.parameters()) + list(self._network.iu_domain_transform.parameters())
        dis_params = list(self._network.ins_discriminator.parameters()) + list(self._network.uni_discriminator.parameters())
        optimizer_gen = optim.SGD(gen_params, lr=self.dot_lr, momentum=0.9, weight_decay=self.weight_decay)
        optimizer_dis = optim.SGD(dis_params, lr=self.dot_lr, momentum=0.9, weight_decay=self.weight_decay)
        scheduler_gen = optim.lr_scheduler.CosineAnnealingLR(optimizer_gen, T_max=self.dot_epochs, eta_min=self.min_lr)
        scheduler_dis = optim.lr_scheduler.CosineAnnealingLR(optimizer_dis, T_max=self.dot_epochs, eta_min=self.min_lr)

        total_gen_params = sum(p.numel() for p in gen_params)
        total_dis_params = sum(p.numel() for p in dis_params)
        logging.info(f"Transform Trainable parameter count: {total_gen_params} (gen) {total_dis_params} (dis)")
        ...
        for i, (_, inputs, targets) in enumerate(train_loader):
            ...
            with torch.no_grad():
                output = self._network(inputs, task_id=self._cur_task, train=True)
                ins_feature = output["pre_logits"]
                uni_feature = output["raw_features"]

            ins_features.append(ins_feature)
            uni_features.append(uni_feature)
            all_targets.append(targets)

        #TODO use the features from previous tasks?
        ins_features = torch.cat(ins_features, dim=0).detach()
        uni_features = torch.cat(uni_features, dim=0).detach()
        all_targets = torch.cat(all_targets, dim=0).detach()
        domain_id = torch.ones(ins_features.size(0), device=self._device).long() * self._cur_domain

        prog_bar = tqdm(range(self.dot_epochs), desc="Transform training")
        for _, epoch in enumerate(prog_bar):
            sf_indexes = torch.randperm(ins_features.size(0))
            ins_features = ins_features[sf_indexes]
            uni_features = uni_features[sf_indexes]
            all_targets = all_targets[sf_indexes]
            domain_id = domain_id[sf_indexes]

            losses_gen, losses_dis = 0.0, 0.0
            losses_gen_adv, losses_gen_cycle = 0.0, 0.0
            losses_fake_mse, losses_cycle_mse = 0.0, 0.0
            losses_fake_ce, losses_cycle_ce = 0.0, 0.0
            correct_ins, correct_uni = 0, 0
            total_ins, total_uni = 0, 0

            num_batches = math.ceil(ins_features.size(0) / self.batch_size)
            for inp_id in range(num_batches):
                start = inp_id * self.batch_size
                end = (inp_id + 1) * self.batch_size
                ins_inp = ins_features[start:end]
                uni_inp = uni_features[start:end]
                targets = all_targets[start:end]
                dom_id = domain_id[start:end]

                # train the discriminator
                self._network.ins_discriminator.requires_grad_(True)
                self._network.uni_discriminator.requires_grad_(True)
                self._network.ui_domain_transform.requires_grad_(False)
                self._network.iu_domain_transform.requires_grad_(False)

                with torch.no_grad():
                    fake_uni = self._network.ins_to_uni(ins_inp, domain_id=dom_id)
                    fake_ins = self._network.uni_to_ins(uni_inp, domain_id=dom_id)
                
                real_ins = ins_inp.detach()
                real_uni = uni_inp.detach()
                all_ins = torch.cat([real_ins, fake_ins], dim=0)
                all_uni = torch.cat([real_uni, fake_uni], dim=0)

                batch_size = real_ins.size(0)
                labels_ins = torch.cat([torch.ones(batch_size), torch.zeros(batch_size)], dim=0).long().to(self._device)
                labels_uni = torch.cat([torch.ones(batch_size), torch.zeros(batch_size)], dim=0).long().to(self._device)

                logit_ins = self._network.ins_discriminator(all_ins)
                logit_uni = self._network.uni_discriminator(all_uni)

                loss_dis_ins = F.cross_entropy(logit_ins, labels_ins)
                loss_dis_uni = F.cross_entropy(logit_uni, labels_uni)
                loss_dis = (loss_dis_ins + loss_dis_uni) / 2
                ...
                _, preds_ins = torch.max(logit_ins, dim=1)
                _, preds_uni = torch.max(logit_uni, dim=1)
                correct_ins += preds_ins.eq(labels_ins.expand_as(preds_ins)).cpu().sum()
                correct_uni += preds_uni.eq(labels_uni.expand_as(preds_uni)).cpu().sum()
                total_ins += all_ins.size(0)
                total_uni += all_uni.size(0)

                # train the transformation
                self._network.ins_discriminator.requires_grad_(False)
                self._network.uni_discriminator.requires_grad_(False)
                self._network.ui_domain_transform.requires_grad_(True)
                self._network.iu_domain_transform.requires_grad_(True)

                fake_uni = self._network.ins_to_uni(ins_inp, domain_id=dom_id)
                fake_ins = self._network.uni_to_ins(uni_inp, domain_id=dom_id)

                pred_fake_ins = self._network.ins_discriminator(fake_ins)
                pred_fake_uni = self._network.uni_discriminator(fake_uni)
                
                loss_gen_adv_ins = F.cross_entropy(pred_fake_ins, torch.ones(batch_size, device=self._device).long())
                loss_gen_adv_uni = F.cross_entropy(pred_fake_uni, torch.ones(batch_size, device=self._device).long())
                loss_gen_adv = (loss_gen_adv_ins + loss_gen_adv_uni) / 2

                cycle_ins = self._network.uni_to_ins(fake_uni, domain_id=dom_id)
                cycle_uni = self._network.ins_to_uni(fake_ins, domain_id=dom_id)

                pred_cycle_ins = self._network.ins_discriminator(cycle_ins)
                pred_cycle_uni = self._network.uni_discriminator(cycle_uni)
                
                loss_gen_cycle_ins = F.cross_entropy(pred_cycle_ins, torch.ones(batch_size, device=self._device).long())
                loss_gen_cycle_uni = F.cross_entropy(pred_cycle_uni, torch.ones(batch_size, device=self._device).long())
                loss_gen_cycle = (loss_gen_cycle_ins + loss_gen_cycle_uni) / 2

                mse_fake_ins = F.mse_loss(fake_ins, ins_inp)
                mse_fake_uni = F.mse_loss(fake_uni, uni_inp)
                loss_fake_mse = (mse_fake_ins + mse_fake_uni) / 2

                mse_cycle_ins = F.mse_loss(fake_ins, ins_inp)
                mse_cycle_uni = F.mse_loss(fake_uni, uni_inp)
                loss_cycle_mse = (mse_cycle_ins + mse_cycle_uni) / 2

                logits_fake_ins = self._network(fake_ins, head_only=True)['logits']
                logits_cycle_ins = self._network(cycle_ins, head_only=True)['logits']

                loss_fake_ce = F.cross_entropy(logits_fake_ins, targets)
                loss_cycle_ce = F.cross_entropy(logits_cycle_ins, targets)

                loss_gen = loss_gen_adv + loss_gen_cycle + loss_fake_mse + loss_cycle_mse + loss_fake_ce + loss_cycle_ce

                optimizer_gen.zero_grad()
                loss_gen.backward()
                optimizer_gen.step()
                losses_gen += loss_gen.item()
                losses_gen_adv += loss_gen_adv.item()
                losses_gen_cycle += loss_gen_cycle.item()
                losses_fake_mse += loss_fake_mse.item()
                losses_cycle_mse += loss_cycle_mse.item()
                losses_fake_ce += loss_fake_ce.item()
                losses_cycle_ce += loss_cycle_ce.item()

            scheduler_gen.step()
            scheduler_dis.step()

            avg_loss_gen = losses_gen / num_batches
            avg_loss_dis = losses_dis / num_batches
            avg_loss_gen_adv = losses_gen_adv / num_batches
            avg_loss_gen_cycle = losses_gen_cycle / num_batches
            avg_loss_fake_mse = losses_fake_mse / num_batches
            avg_loss_cycle_mse = losses_cycle_mse / num_batches
            avg_loss_fake_ce = losses_fake_ce / num_batches
            avg_loss_cycle_ce = losses_cycle_ce / num_batches
            train_acc_ins = np.around(tensor2numpy(correct_ins) * 100 / total_ins, decimals=2)
            train_acc_uni = np.around(tensor2numpy(correct_uni) * 100 / total_uni, decimals=2)

            info = "Task {}, Epoch {}/{} => Loss_gen {:.3f}=adv(fake {:.3f}+cycle {:.3f})+mse(fake {:.3f}+cycle {:.3f})+ce(fake {:.3f}+cycle {:.3f}), Loss_dis {:.3f}, Train_acc_ins {:.2f}%, Train_acc_uni {:.2f}%".format(
                self._cur_task,
                epoch + 1,
                self.dot_epochs,
                avg_loss_gen,

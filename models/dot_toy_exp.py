...
class Learner(BaseLearner):
    def __init__(self, args:dict):
        ...
        self.enable_toy_experiment = args.get("enable_toy_experiment", False)
        if self.enable_toy_experiment:
            self.ins_cls_dists_all_domains = {}
            self.uni_cls_dists_all_domains = {}
    ...
    def incremental_train(self, data_manager: Union[DataManager, DomainDataManager] = None):
        ...
        if self.enable_toy_experiment:
            self._compute_distributions_all_domains(data_manager)
        if self.dot_epochs > 0:
            self._train_transformation() # train u->i and i->u transformations
        if self.ca_epochs > 0:
            if self.enable_toy_experiment:
                self._train_classifier_toy(self.test_loader)
            else:
    ...
    def _compute_distributions_all_domains(self, data_manager: DomainDataManager):
        if len(self._multiple_gpus) > 1:
            self._network.module.backbone.eval()
            self._network.module.original_backbone.eval()
        else:
        ...
        dataset = data_manager.get_domain_dataset(
            np.arange(self._known_classes, self._total_classes),
            source="train", mode="train", domain_id='all',
        )
        ...
        # for class distribution across all domains
        features = []
        raw_features = []
        labels = []
        for i, (_, inputs, targets) in enumerate(data_loader):
        ...
        for cls_label in unique_labels:
            ...
            new_ins_distribution = MultiCentroidDist(n_centroids=self.num_class_centroids, feature_dim=cls_feature.shape[-1], device=self._device)
            new_ins_distribution.compute_centroids(cls_feature)
            self.ins_cls_dists_all_domains[cls_label_id] = new_ins_distribution

            new_uni_distribution = MultiCentroidDist(n_centroids=self.num_class_centroids, feature_dim=cls_raw_feature.shape[-1], device=self._device)
            new_uni_distribution.compute_centroids(cls_raw_feature)
            self.uni_cls_dists_all_domains[cls_label_id] = new_uni_distribution
        logging.info(f"Distributions computed for {unique_labels.shape[0]} classes across all domains: {unique_labels.tolist()} ")
    ...
    def _train_classifier_toy(self, test_loader):
        if len(self._multiple_gpus) > 1:
            self._network = self._network.module
        self._network.back_up_head()
        for p in self._network.rec_head.parameters():
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            for c_id in range(self._total_classes):
                ...
                ins_dist: MultiCentroidDist = self.ins_cls_dists_all_domains[c_id]
                # uni_dist: MultiCentroidDist = self.uni_cls_dists_all_domains[c_id]
            ...
            sf_indexes = torch.randperm(ins_feature.size(0))
            # sf_indexes = torch.randperm(uni_feature.size(0))
            ins_feature = ins_feature[sf_indexes]
            # uni_feature = uni_feature[sf_indexes]
            label = label[sf_indexes]
            ...
            for _iter in range(self._total_classes):
                ...
                logit_ins = self._network(ins_inp, head_only=True)['logits']
                logit_ins = self.logit_normalize(logit_ins)
                loss_ins = F.cross_entropy(logit_ins, tgt)
                loss = loss_ins
                ...
                _, preds = torch.max(logit_ins, dim=1)
                # _, preds = torch.max(logit_uni, dim=1)
                correct += preds.eq(tgt.expand_as(preds)).cpu().sum()
                total += len(tgt)
            ...
            if (epoch + 1) % 5 == 0 or epoch == self.ca_epochs - 1:
                test_acc = self._compute_accuracy(self._network, test_loader, use_uninstructed=False)
    ...
    def _eval_cnn(self, loader):
        ...
        for _, (_, inputs, targets) in enumerate(loader):
            ...
            with torch.no_grad():
                # FIXME check here whether use uninstructed feature or not

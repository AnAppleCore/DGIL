...
from utils.inc_net import SPromptVitNet
...
class Learner(BaseLearner):
    ...
    def __init__(self, args):
        ...
        self._network = SPromptVitNet(args, pretrained=True)
        ...
        self.single_head = args.get("single_head", False)
    ...
    def _train(self, train_loader, test_loader):
        ...
        for name, param in self._network.named_parameters():
            ...
            try:
                if "classifier_pool" + "." + str(self._network.module.current_task - 1) in name:
                    param.requires_grad_(True)
                if "prompt_pool" + "." + str(self._network.module.current_task - 1) in name:
                    param.requires_grad_(True)
                if "classifier" in name and self.single_head:
                    param.requires_grad_(True)
            except:
                if "classifier_pool" + "." + str(self._network.current_task - 1) in name:
                    param.requires_grad_(True)
                if "prompt_pool" + "." + str(self._network.current_task - 1) in name:
                    param.requires_grad_(True)
                if "classifier" in name and self.single_head:
                    param.requires_grad_(True)
    ...
    def train_function(self, train_loader, test_loader, optimizer, scheduler):
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            for i, (_, inputs, targets) in enumerate(train_loader):
                ...
                logits = self._network(inputs)['logits'] # given current task id
                if self.single_head:
            ...
            test_acc = self._compute_accuracy_domain(self._network, test_loader) # given currrent task id

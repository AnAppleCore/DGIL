...
def compute_domain_mean(class_means, class_num_samples):
    total_samples = sum(class_num_samples)
    domain_mean = torch.zeros_like(class_means[0])
    for mean, num_samples in zip(class_means, class_num_samples):
        domain_mean += mean * num_samples
    domain_mean /= total_samples
    return domain_mean


def compute_domain_cov(class_means, class_covs, class_num_samples, domain_mean):
    total_samples = sum(class_num_samples)
    domain_cov = torch.zeros_like(class_covs[0])
    for mean, cov, num_samples in zip(class_means, class_covs, class_num_samples):
        domain_cov += cov * num_samples
        diff = mean - domain_mean
        domain_cov += num_samples * torch.outer(diff, diff)
    domain_cov /= total_samples
    return domain_cov


def compute_transform_mtx(source_mean, source_cov, target_mean, target_cov):
    source_cov_reg = source_cov + 1e-4 * torch.eye(source_cov.shape[0], device=source_cov.device)
    source_cov_inv = torch.inverse(source_cov_reg)
    A = torch.matmul(target_cov, source_cov_inv)
    b = target_mean - torch.matmul(A, source_mean)
    return A, b